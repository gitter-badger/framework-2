        ...
        if($input[0]=='text' || $input[0]=='password' || $input[0]=="file") {
            if(is_array($valu)) {
                foreach($valu as $k=>$v) {
                    ...
                    $fld.="$lable<input type=\"$input[0]\" id=\"".$input[1].$i1."\" name=\"".$input[1]."[$i1]\" $autoc value=\"$v\" class=\"#c$tm#\" #o$tm# />";
                    $i1++;
                }
            }else {
                $lable='<label for="'.$input[1].'">'.$label_caption[0].'</label>';
                $fld.="$lable<input type=\"$input[0]\" id=\"".$input[1]."\" name=\"".$input[1]."\" $autoc value=\"$valu\" class=\"#c$tm#\" #o$tm# />";
            }
            ...
            if(is_array($valu)) {
                foreach($valu as $k=>$v) {
                    ...
                    $fld.="$lable<button type=\"$input[0]\" id=\"".$input[1].$i1."\" name=\"".$input[1]."[$i1]\" $autoc value=\"$v\" class=\"$input[4]\" #o$tm# >$v</button>";
                    $i1++;
                }
            }else {
                if($is_style) $_Validate_forms_style[$input[1]]='$("#'.$input[1].'").button({icons: {primary: "'.$icon_option[0].'",secondary: "'.$icon_option[1].'"}});';
                $fld.="$lable<button type=\"$input[0]\" id=\"".$input[1]."\" name=\"".$input[1]."\" value=\"$valu\" class=\"$input[4]\" #o$tm# >$valu</button>";
            }
            ...
            if(is_array($valu)) {
                foreach($valu as $k=>$v) {
                    ...
                    $fld.="$lable<input type=\"$input[0]\" id=\"".$input[1].$i1."\" name=\"".$input[1].$addToName."\" $autoc value=\"$v\" class=\"$input[4]\" #o$tm# />";
                    $i1++;
                }
            }else {
                if($is_style) $_Validate_forms_style[$input[1]]='$("#'.$input[1].'").button({icons: {primary: "'.$icon_option[0].'",secondary: "'.$icon_option[1].'"}});';
                $lable='<label for="'.$input[1].'">'.$label_caption[0].'</label>';
                $fld.="$lable<input type=\"$input[0]\" id=\"".$input[1]."\" name=\"".$input[1]."\" value=\"$valu\" class=\"$input[4]\" #o$tm# />";
            }
            ...
            if(is_array($valu)) {
                foreach($valu as $k=>$v) {
                    ...
                    $v=str_replace(':on', '', $v);
                    $optn.='<option value="'.$v1.'">'.$v.'</option>';
                }
            }else {
                $v1=str_replace(':on', '" selected="selected', $valu);
                $v=str_replace(':on', '', $valu);
                $optn.='<option value="'.$v1.'">'.$v.'</option>';
            }
            $fld.="$lable<select id=\"".$input[1]."\" $stselect name=\"".$input[1]."\" class=\"$input[4]\" #o$tm# />$optn</select>";

        }else {
            $lable='<label for="'.$input[1].'">'.$label_caption[0].'</label>';
            $fld="$lable<$input[0] name=\"$input[1]\" id=\"$input[1]\" class=\"$input[4]\" #o$tm#>$valu</$input[0]>";
        }
...
/*Generate JS code */
    ...
    write_JScode('$(document).ready(function() {'.$js_validate_document.'});'); 
    return  $fld ;
}
