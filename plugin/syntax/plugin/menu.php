...
function SYNTAX_menu($template) {
    $TEMPLATE_link = SITE_PATH . TEMPLATE_PATH . 'urorbit/';
    $template = trim(Syntax($template));
    return get_pages_under_Tpage_id($template, 0, "page_place like '%$template%' and ");
}
...
function get_pages_under_Tpage_id($tem, $id = 0, $opt = '') {
$COLUMNS_ON_PAGES[]='id';    
...
global $memarea;
    ...
    $result = getResults("Select * From pages where $opt slave='$id' and page_active=1 $memarea ORDER BY page_sort  ASC");
    if (is_array($result))
        foreach ($result as $row) {
            unset($old_var);
            unset($new_var);
            if ($row[id] == $_GET[id] || $row[id] == get_root_page_id($_GET[id]))
                $new_var[0] = 'current active'; else
                $new_var[0] = '';
            $old_var[0] = "%current%";            
                foreach ($COLUMNS_ON_PAGES as $field) {
                    if ($field == "image")
                        $row[$field] = "/?image_thump=" . $row[id];
