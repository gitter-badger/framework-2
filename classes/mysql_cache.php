...
function getResults($query) {
    ...
    $cachefile = SITE_PATH . 'cache/cacheSQLfile_' . md5($query);
//exec("find " . SITE_PATH . "cache/cacheSQLfile_* -mmin +$secondtomin -exec rm {} \;");
...
function jsUA($ua, $js_array = array(), $compressjs = 10) {
    $dir = SITE_PATH . TEMPLATE_PATH;
    ksort($js_array);
    $jsau = '';

    foreach ($js_array as $createjsfile_row)
        $jsau.= addslashes(file_get_contents($dir . $createjsfile_row));
    ...
    /* create a new file */
    $handle = fopen(SITE_PATH . "cache/{$ua}" . get_cache('jshash' . $ua) . ".js", 'w');
    fwrite($handle, $jsau);
    fclose($handle);
...
function createjsfile($compressjs = 0) {
    ...
    if ($jshash2 == get_cache('jshash2') && file_exists(SITE_PATH . "cache/2{$jshash2}.js")) {
        $jshash2 = '';
    } else {
        @unlink(SITE_LINK . "cache/2" . get_cache('jshash2') . ".js");
    ...
    if ($jshash1 == get_cache('jshash1') && file_exists(SITE_PATH . "cache/1{$jshash1}.js")) {
        $jshash1 = '';
    } else {
    ...
    return SITE_LINK . "cache/1" . get_cache('jshash1') . ".js";
}
...
function compress_css($file_name) {
    ...
    preg_match("|template/(.*)/css|U", $file_name, $x);
    $_FCSSILE = $x[1];
    $ltrrtl = get_cache('language');
    return str_ireplace('imagedata', 'url(data:', str_ireplace('url(/template/' . $_FCSSILE . '/images/http://', 'url(http://', str_ireplace('{}', '{ }', str_ireplace('url(', 'url(/template/' . $_FCSSILE . '/images/', str_ireplace('url(data:', 'imagedata', str_ireplace(array('; ', ' }', '{ ', ': ', ' {', '  '), array(';', '}', '{', ':', '{', ' '), str_ireplace(array("\r\n", "\r", "\n", "\t", '  '), ' ', $cssx)))))));
}
...
function createcssfile($e = 0) {
    ...
    if ($csshash == get_cache('csshash') && file_exists(SITE_PATH . "cache/{$csshash}.css")) {
        return '';
    }
    ...
    $handle = fopen(SITE_PATH . "cache/{$csshash}.css", 'w');
    fwrite($handle, '@charset "utf-8";' . "\n" . $_SESSION['@import_media'] . CHNG_LANGUAGE($cssdone));
    fclose($handle);
    return SITE_LINK . "cache/{$csshash}.css";
}
